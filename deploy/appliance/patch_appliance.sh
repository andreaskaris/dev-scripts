...
# The original script exits immediately if the directory doesn't exist.
# On bare metal the agent starts late; Restart=on-failure handles retries
# but only if we exit 1, which this patched version does after MAX_WAIT s.
